  ...
  if (message.type === 'quote' && message.quote) {
    ...
    return (
      <View style={[styles.row, styles.rowLeft]}>
        <View style={[styles.quoteCard, { backgroundColor: colors.card, borderColor: colors.primary }]}>
          <View style={styles.quoteHeader}>
            <FileText size={18} color={colors.primary} />
            <Text style={[styles.quoteTitle, { color: colors.text }]}>Devis</Text>
            {quote.status === 'accepted' ? <Badge label="Accepté" tone="success" /> : null}
            {quote.status === 'declined' ? <Badge label="Refusé" tone="neutral" /> : null}
          </View>
          <Text style={[styles.quoteAmount, { color: colors.primary }]}>
            {formatPrice(quote.amount)}
          </Text>
          <Text style={[styles.quoteDetails, { color: colors.textSecondary }]}>{quote.details}</Text>
          {message.text ? (
...
const styles = StyleSheet.create({
  ...
  systemRow: { alignItems: 'center', marginVertical: Spacing.two, paddingHorizontal: Spacing.four },
